(() => {
  ...
  function setSocialLinks(telegramUrl = FALLBACK_TELEGRAM, instagramUrl = FALLBACK_INSTAGRAM) {
    ...
    document.querySelectorAll('a').forEach(link => {
      ...
      if (href.includes('t.me/') || text.includes('telegram') || label.includes('telegram')) { link.href = tg; link.target = '_blank'; link.rel = 'noopener noreferrer'; }
      if (href.includes('instagram.com/') || text.includes('instagram') || label.includes('instagram')) { link.href = ig; link.target = '_blank'; link.rel = 'noopener noreferrer'; }
    });
  }
  ...
  async function loadStoreSettings(client) {
    ...
    try {
      ...
      if (data.store_name) { document.querySelectorAll('.brand strong,.footer-brand').forEach(el => el.textContent = data.store_name); document.title = `${data.store_name} — авторская флористика в Москве`; }
      if (data.phone) { const cleanPhone = data.phone.replace(/[^+\d]/g, ''); document.querySelectorAll('a[href^="tel:"]').forEach(el => { el.href = `tel:${cleanPhone}`; if (el.textContent.trim()) el.textContent = data.phone; }); }
  ...
  function guardAgainstLegacyCatalog(client) {
    const grid = document.querySelector('.product-grid');
  ...
  async function selectProduct(productId) {
    ...
    if (!client) { const fallback = FALLBACK_PRODUCTS.find(p => String(p.id) === String(productId)); if (fallback) renderOrderForm(fallback); return; }
    try { const { data, error } = await client.from('products').select('id,name,price,image_url').eq('id', productId).eq('is_active', true).maybeSingle(); if (error || !data) throw error || new Error('Букет недоступен'); renderOrderForm(data); }
    catch (error) { const fallback = FALLBACK_PRODUCTS.find(p => String(p.id) === String(productId)); if (fallback) renderOrderForm(fallback); }
  }
  ...
  function bindInteractions() {
    ...
    document.addEventListener('click', event => {
      ...
      if (button) { event.preventDefault(); selectProduct(button.dataset.productId); return; }
