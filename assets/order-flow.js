(() => {
  ...
  const FALLBACK_INSTAGRAM = 'https://www.instagram.com/smflowers.msk?igsh=enQqZGtuaW1qNWRi&utm_source=qr';
  ...
  function renderProducts(products) {
    ...
    grid.innerHTML = products.map((product, index) => `
      <article class="product">
        <div class="product-image"><span class="product-number">${String(index + 1).padStart(2, '0')}</span><img src="${escapeHtml(product.image_url || `assets/bouquet-${(index % 6) + 1}.svg`)}" alt="${escapeHtml(product.name)}" loading="lazy"></div>
        <div class="product-info"><div><h3>${escapeHtml(product.name)}</h3><p>${escapeHtml(product.description || '')}</p><p><strong>${Number(product.price).toLocaleString('ru-RU')} ₽</strong></p></div><button class="order-button" type="button" data-product-id="${escapeHtml(product.id)}">Заказать</button></div>
      </article>`).join('');
  }
  ...
  async function loadProducts(client) {
    ...
    renderProducts(error || !data?.length ? FALLBACK_PRODUCTS : data);
  }
  ...
  function renderOrderForm(product) {
    const modal = document.getElementById('order-modal'); const actions = modal?.querySelector('.modal-actions');
    if (!modal || !actions) return;
    selectedProduct = product;
    const selected = document.getElementById('selected-bouquet'); if (selected) selected.textContent = `Вы выбрали: «${product.name}»`;
    ...
    document.getElementById('public-order-form').addEventListener('submit', submitOrder); openOrderModal();
  }

  async function submitOrder(event) {
    event.preventDefault(); const client = getClient(); if (!client || !selectedProduct) return;
    ...
    try {
      const { data: orderId, error } = await client.rpc('create_public_order', { p_customer_name:String(data.get('name')||'').trim(), p_phone:String(data.get('phone')||'').trim(), p_email:String(data.get('email')||'').trim()||null, p_telegram:null, p_instagram:null, p_delivery_address:String(data.get('address')||'').trim()||null, p_delivery_date:String(data.get('date')||'')||null, p_delivery_time:String(data.get('time')||'').trim()||null, p_comment:String(data.get('comment')||'').trim()||null, p_items:[{product_id:selectedProduct.id,quantity:1}] });
    ...
    } catch (error) { msg.textContent = error?.message || 'Не удалось оформить заказ. Попробуйте ещё раз.'; msg.style.color = '#b3261e'; submit.disabled = false; submit.textContent = 'Оформить заказ'; }
  }
  ...
  async function selectProduct(productId) {
    ...
    if (error || !data) { const fallback = FALLBACK_PRODUCTS.find(p => String(p.id) === String(productId)); if (fallback) renderOrderForm(fallback); return; }
    renderOrderForm(data);
  }

  function bindInteractions() { document.addEventListener('click', event => { const button = event.target.closest('[data-product-id]'); if (button) { event.preventDefault(); selectProduct(button.dataset.productId); } }); }

  async function init() { const client = getClient(); if (!client) { setSocialLinks(); setTimeout(init, 300); return; } bindInteractions(); setSocialLinks(); await Promise.all([loadStoreSettings(client), loadProducts(client)]); }
