/* Live site content: each CMS resource is isolated so one unavailable endpoint never blanks the whole page. */
(()=>{
  ...
  const textMap={
    ...
    'Коллекции':[['.collections .section-heading .eyebrow','collections_eyebrow'],['#collections-title','collections_title'],['.section-heading a.text-link','collections_link']],
    'На заказ':[['.statement-copy .eyebrow','custom_eyebrow'],['#custom-title','custom_title'],['.statement-copy p','custom_text'],['.statement-copy .text-link','custom_link']],
    'Букеты':[['.products .section-heading .eyebrow','bouquets_eyebrow'],['#products-title','bouquets_title'],['.products .section-heading > p','bouquets_note']],
    'Философия':[['.values-intro .eyebrow','values_eyebrow'],['#values-title','values_title'],['.values-intro > p:last-child','values_intro']],
    'Жизнь студии':[['.gallery .section-heading .eyebrow','life_eyebrow'],['.gallery .section-heading .text-link','life_instagram_button']],
    'Финальный блок':[['.final-copy .eyebrow','final_eyebrow'],['#final-title','final_title'],['.final-copy > p','final_text'],['.final-actions .button','final_call_button'],['.final-actions .button--outline','final_telegram_button']],
    'Контакты':[['.footer-brand','footer_brand'],['.footer-column:nth-child(1) h3','footer_nav_title'],['.footer-column:nth-child(2) h3','footer_contacts_title'],['.footer-column:nth-child(3) h3','footer_studio_title'],['.footer-column:nth-child(3) p:first-of-type','studio_address'],['.footer-column:nth-child(3) p:last-of-type','studio_hours'],['.footer-bottom span:first-child','footer_copyright'],['.footer-bottom span:last-child','footer_tagline']]
  };
  const mediaKey=slot=>({hero:'hero',custom:'custom_art',final:'final_art'}[slot]||slot);
  const get=async(p,label)=>{try{return await json(p)}catch(e){console.warn(`SM Flowers: ${label} unavailable`,e);return null}};
  async function run(){
    const [texts,media,products]=await Promise.all([
      get('/rest/v1/site_content?select=section,content_key,content,is_visible&is_visible=eq.true&order=sort_order.asc','site text'),
      get('/rest/v1/site_media?select=section,slot_key,image_url,alt_text,is_visible&is_visible=eq.true&order=sort_order.asc','site media'),
      get('/rest/v1/products?select=id,name,description,price,image_url,is_active,is_featured,sort_order&is_active=eq.true&order=sort_order.asc','products')
    ]);
    if(Array.isArray(texts)) for(const row of texts) for(const [selector,key] of (textMap[row.section]||[])) if(row.content_key===key)setText(selector,row.content);
    if(Array.isArray(media)){
      const by=(section,slot)=>media.find(x=>x.section===section&&x.slot_key===mediaKey(slot))?.image_url;
      const setImg=(selector,url,fallback)=>{const e=document.querySelector(selector);if(!e)return;if(url)e.src=mediaUrl(url);if(fallback)e.onerror=()=>{e.onerror=null;e.src=fallback}};
      setImg('.hero-visual img',by('Главная','hero'),'/media/hero.jpg');
      setImg('.statement-art img',by('На заказ','custom'),'/media/custom.jpg');
