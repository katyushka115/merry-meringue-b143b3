(() => {
  ...
  const INSTAGRAM = 'https://www.instagram.com/smflowers.msk?igsh=enFqZGtuaI1qNWRi&utm_source=qr';
  ...
  function client(){if(window.smSupabase?.from)return window.smSupabase;if(window.supabase?.createClient){window.smSupabase=window.supabase.createClient(URL,KEY);return window.smSupabase;}return null;}
  ...
  function prepareImage(img,rawUrl,priority=false){if(!img)return Promise.resolve(false);if(!rawUrl)return Promise.resolve(true);const width=Math.min(Math.max(Math.round((window.innerWidth||1200)*(window.devicePixelRatio||1)),480),1800);img.loading=priority?'eager':'lazy';img.decoding='async';if(priority)img.fetchPriority='high';img.dataset.smRawUrl=rawUrl;const optimized=optimizedImageUrl(rawUrl,width);return new Promise(resolve=>{const show=(src)=>{img.src=src;img.classList.add('sm-dynamic-photo-ready');resolve(true);};const preload=new Image();preload.decoding='async';preload.onload=()=>show(optimized);preload.onerror=()=>{const fallback=new Image();fallback.onload=()=>show(rawUrl);fallback.onerror=()=>resolve(false);fallback.src=rawUrl;};preload.src=optimized;});}

  let collectionLabels=[];
  function enforceCollectionLabels(){
    const cards=document.querySelectorAll('.collection-card');
    collectionLabels.forEach((label,i)=>{const title=cards[i]?.querySelector('.collection-meta h3');if(title&&title.textContent!==label)title.textContent=label;});
  }
  function watchCollectionLabels(){
    if(window.__smCollectionLabelObserver)return;
    const observer=new MutationObserver(()=>enforceCollectionLabels());
    observer.observe(document.body,{subtree:true,childList:true,characterData:true});
    window.__smCollectionLabelObserver=observer;
    [0,100,500,1000,2000,4000,8000].forEach(ms=>setTimeout(enforceCollectionLabels,ms));
  }
  ...
  async function applyMedia(rows){const by=Object.fromEntries(rows.map(r=>[r.slot_key,r]));const jobs=[];
    ...
    const cards=document.querySelectorAll('.collection-card');['collection_1','collection_2','collection_3','collection_4'].forEach((k,i)=>{const row=by[k];if(cards[i]){const title=cards[i].querySelector('.collection-meta h3');if(title&&row?.label){const label=String(row.label).replace(/^Коллекция:\s*/i,'');collectionLabels[i]=label;setText(title,label);}const caption=cards[i].querySelector('.collection-meta span');if(caption&&row?.label)caption.textContent=`${String(i+1).padStart(2,'0')} / коллекция`;const img=cards[i].querySelector('img');if(img&&row?.alt_text)img.alt=row.alt_text;jobs.push(prepareImage(img,row?.image_url));}});
    watchCollectionLabels();
    jobs.push(prepareImage(document.querySelector('.statement-art img'),by.custom_art?.image_url));
    const gallery=document.querySelectorAll('.gallery-item img');const keys=['life_photo_1','life_photo_2','life_photo_3','life_photo_4','gallery_photo_1','gallery_photo_2','gallery_photo_3','gallery_photo_4'];keys.forEach((k,i)=>{const row=by[k];if(gallery[i])jobs.push(prepareImage(gallery[i],row?.image_url));});jobs.push(prepareImage(document.querySelector('.final-art img'),by.final_art?.image_url));await Promise.all(jobs);enforceCollectionLabels();}
