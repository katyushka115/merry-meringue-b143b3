(() => {
  ...
  function applyFallbackContent() {
    const defaults = {
      ...
      '.value-item:nth-child(3) p': 'Бесплатная доставка от 1 часа',
      '#gallery-title': '@smflowers.msk',
      '.footer-column:nth-child(4) p:first-of-type': 'Москва, Ленинский проспект, 94А'
    };
    ...
    if (address) {
      ...
      setHref(link, MAP); link.textContent = '📍 Построить маршрут →';
    }
  }
  ...
  function applyContent(rows) {
    ...
    // These production values are intentionally authoritative even if old values remain in site_content.
    ...
    if (address) {
      let link = address.parentElement.querySelector('.sm-studio-route-link');
      if (!link) { link = document.createElement('a'); link.className = 'sm-studio-route-link'; address.parentElement.appendChild(link); }
      setHref(link, m.studio_map_url?.content || MAP);
      link.textContent = '📍 ' + (m.studio_map_label?.is_visible ? m.studio_map_label.content : 'Построить маршрут →');
    }
    const instagram = m.life_instagram_url?.content || INSTAGRAM;
    const telegram = m.final_telegram_url?.content || TELEGRAM;
    document.querySelectorAll('a').forEach(a => {
      const href = (a.getAttribute('href') || '').toLowerCase(), label = (a.textContent || '').toLowerCase();
      if (href.includes('instagram.com') || label.includes('instagram')) setHref(a, instagram);
      if (href.includes('t.me') || href.includes('telegram') || label.includes('telegram')) setHref(a, telegram);
    });
  }
  ...
  async function loadCollections() {
    ...
    try {
      ...
      data.slice(0, cards.length).forEach((category, index) => { const card = cards[index], title = card.querySelector('.collection-meta h3'), img = card.querySelector('img'); if (title) setText(title, category.name); if (img) { if (category.image_url) img.src = proxifyImage(category.image_url); img.alt = `${category.name} — коллекция букетов`; img.loading = 'eager'; img.decoding = 'async'; } });
